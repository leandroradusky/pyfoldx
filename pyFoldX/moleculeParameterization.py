...
class JSonMolecule( object ):
    ...
    def toJson(self, toFile=""):
        ...
        if toFile == "":
            ...
            return FileHandler.writeLine(toFile,simplejson.dumps(retdict, indent="\t"))
    
    @staticmethod
    def fromJson(filePath):
        loaded_dict = simplejson.loads("\n".join(FileHandler.getLines(filePath)))
...
def loadMappings(listfile):
    try:
        ...
        for line in FileHandler.getLines(listfile):
            ...
            retDict[fields[0]] = (fields[1], fields[2])
    except:
...
def parameterize(mol, mappingsDict):
    
    JsonMol = JSonMolecule(mol.code)
    JsonMol.setThreeLetterCode(mol.code)
    
    print( "Mappings loaded:" )
    for atom in mappingsDict: 
        print( "Atom %s mapped to atom %s of molecule %s" % (atom, mappingsDict[atom][0], mappingsDict[atom][1]) )
    
    if not checkListFile(mol,mappingsDict):
    ...
    for atom in mappingsDict:
        for table in ["H_Pos", "solvenergy", "hbondinfo", "xyz_water"]:
            ...
            records = JSonParameterLoader.getRecordsInTable(table,mappingsDict[atom][1],mappingsDict[atom][0])
            ...
            for record in records:
                ...
                if table=="solvenergy":
                    ...
                    moreneigh = []
                    #for a in record["neigh_atom"]:
                    #    moreneigh += mol.atoms[a]["conect"]
                    
                    #record["neigh_atom"]+=moreneigh
                ...
                if table=="H_Pos":
                    record["partner1"] = findInListDict(mappingsDict, record["partner1"])
                    record["partner2"] = findInListDict(mappingsDict, record["partner2"])
                    record["group"] = findInListDict(mappingsDict, record["group"])
            ...
            if len(records) == 0 and table == "xyz_water":
                ...
                if mappingsDict[atom][1] in ["A","C","G","T"] and atom[0] == "O":
    ...
    tlc = "SER" #raw_input("Please insert the three letter code of the residue to stole general parameters: ")
...
if __name__ == "__main__":
    ...
    if checkParameters(args):
        mol = loadMoleculeFromPDB(args.molfile)
        ...
        mapsDict = loadMappings(args.list) 
       
        if args.list != None:
            newMol = parameterize(mol, mapsDict)
